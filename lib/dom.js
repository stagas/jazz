...
dom.append = function(el, node, dict) {
  el = dom.getElement(el);
  if ('forEach' in node) node.forEach(dom.append.bind(null, el));
  else if ('views' in node) dom.append(el, node.views);
  else if (dict === true) for (var key in node) dom.append(el, node[key]);
  else el.appendChild(dom.getElement(node));
};
